...
(* ILU3 - Partie Coq - TP 1 *)

(* Rappel de l'URL des supports de Cours :
   https://pfitaxel.github.io/ilu3-coq-alectryon/ *)

(* Le sujet des TPs Coq se présentent sous la forme d'un fichier .v à compléter.
   Vous pouvez coder vos réponses :
   - dans Emacs+ProofGeneral (environnement de développement + général que Coq) :
     https://github.com/erikmd/tapfa-init.el
   - ou dans CoqIDE (environnement de développement Coq basé sur GTK)
 *)

(* N'hésitez pas à solliciter votre encadrant de TP pour toute question. *)

(* Emacs : principaux raccourcis claviers : faire C-f1 (Ctrl+F1) ou M-f1 (Alt+F1)
   Les raccourcis ProofGeneral commencent tous par C-c (Ctrl+C)
   En particulier pour évaluer les phrases Coq : aller jusqu'au curseur
   en faisant "C-c RET" (<=> Ctrl+C Entrée) ou "C-c C-RET" (<=> Ctrl+C Ctrl+Entrée)
   Quelques astuces/suggestions :
   - Si vous venez d'écrire une phrase Coq, vous pouvez aussi appuyer
     2 fois rapidement sur "." (dans ce cas, pas besoin de "C-c RET")
   - Pour aller à la fin de la zone validée par Coq, faire "C-c C-."
   - Pour avancer ou reculer d'un cran, faire "C-c C-n" ou "C-c C-u"
   - Vous pouvez aussi utiliser la barre d'outils Emacs, affichable
     avec le menu (?) -> Afficher / masquer la barre d'outils.
*)
(* CoqIDE : pour avancer ou reculer d'un cran, faire "Ctrl+↓" ou "Ctrl+↑" *)
...
Definition ex0 : B -> B := fun b : B => b.

Definition ex1 : A -> B -> A := fun (a : A) (b : B) => a.

Definition ex2 : A -> B -> B := fun (a : A) (b : B) => b.

Definition ex3 : A -> (A -> B) -> B := fun (a : A) (f : (A -> B)) => f a.

Definition ex4 : (A -> B) -> (B -> C) -> A -> C := fun (f : (A -> B)) (g : (B -> C)) (a : A) => g (f a).

Definition ex5 : (A -> B) -> (A -> B -> C) -> A -> C := fun (f : (A -> B)) (g : (A -> B -> C)) (a : A) => g a (f a).